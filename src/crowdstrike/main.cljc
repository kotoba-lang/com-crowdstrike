(ns crowdstrike.main
  "Kotodama WASM entrypoint for the Crowdstrike clean-room actor (L4) — Clojure port.
  ...
  facts (`crowdstrike.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "crowdstrike")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"crowdstrike.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "crowdstrike-compat" :tier tier :entities entities} 200])
...
(defn start! [] :crowdstrike-compat/ready)
